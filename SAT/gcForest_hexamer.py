...
readHexamer = "/home/chenzhuod/galaxy/tools/CAFU/RSeQC-2.6.5/scripts/read_hexamer.py"
generateBG = "/home/chenzhuod/galaxy/tools/CAFU/SAT/generateBGSeq.py"
...
def plotPR(cvRes, out):
    ...
    for fold in cvRes.keys():
        ...
        plt.plot(recall, precision, lw=1, alpha=0.3, label='PR fold %d (AUC = %0.2f)' % (i, curAUC))
        i += 1
...
def plotMeasures(cvRes, out, beta = 1, threshold = 0.5):
    ...
    ax.barh(ind, y, width, color = ['#FF3333', '#FFCC33', '#99CC33', '#66CC33', '#66CCCC', '#336699'])
...
neg1feacmd = "Rscript " + hexamerScore + " -input " + negSeq + " -target " + negHex + " -bg " \
             + negBGHex + " -out " + negfea1dir
...
cmdfea2 = "Rscript " + hexamerScore + " -input " + candidateSeq + " -target " + posHex + " -bg " \
          + posBGHex + " -out " + canfea2dir
