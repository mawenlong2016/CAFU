...
- Similar to **Upload regular file**, click **Get Data** first (see figure blow):
  ...
  Seleting a list files to upload as a collection (e.g. upload all files with ZIP suffix in the folder ```/your directory/CAFU/test_data/SE RNA-Seq/```):
  ...
  Then enter a name for you collection and click **Create list** to finish.
...
In this section, we will provide an example to show how to use CAFU modules to perform unmapped reads extraction.
- **Quality control**
...
- Characterize Nucleic-acid Feature
  ...
  In this module, two features can be analyzed using this module (see figure below).
